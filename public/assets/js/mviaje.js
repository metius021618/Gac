/**
 * GAC - JavaScript Vista Modo Viaje
 * Consulta correos con asuntos category=modo_viaje (coincidencia exacta).
 */

(function() {
    'use strict';

    const mviajeForm = document.getElementById('mviajeForm');
    ...
    const API_ENDPOINT = '/MViaje';
    const FETCH_TIMEOUT_MS = 25000;

    function init() {
        if (!mviajeForm) return;

        mviajeForm.addEventListener('submit', handleSubmit);
    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            if (data.success) {
                ...
                showError(data.message || 'No se encontraron correos de Modo Viaje.');
            }
        } catch (error) {
    ...
    function setLoadingState(loading) {
        if (loading) {
            ...
            mviajeForm.classList.add('loading');
        } else {
            submitBtn.disabled = false;
            btnLoader?.classList.remove('active');
            if (btnText) btnText.textContent = 'Consultar Modo Viaje';
            mviajeForm.classList.remove('loading');
        }
    }
