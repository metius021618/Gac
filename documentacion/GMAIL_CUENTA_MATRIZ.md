...
2. Ve a **Asignar/Actualizar Contraseña** (o a **Correos** → filtro Gmail).
3. Pulsa **“Conectar Gmail (cuenta matriz)”**.
4. Inicia sesión en Google con la cuenta Gmail que quieres usar como matriz y acepta los permisos de solo lectura.
5. Tras autorizar, volverás a la lista de correos Gmail; la cuenta quedará guardada en `email_accounts` con tipo `gmail`.

No hace falta “asignarla” a ningún usuario: el cron lee **todas** las cuentas Gmail que estén en `email_accounts` con tipo `gmail`.
