<?php
/**
 * GAC - Tabla Correos registrados: ID, Correo, Asignado, Acciones (solo eliminar)
 */
$filter = $filter ?? '';
                ...
                <?php foreach ($email_accounts as $account): ?>
                    ...
                    <tr data-id="<?= (int)$account['id'] ?>">
                        ...
                        <td class="actions-cell text-center">
                            <?php if ($can_delete): ?>
                            <button type="button"
                                    class="btn-icon btn-delete-correo-registrado"
...
$per_page_val = (int)($per_page ?? 15);
$from = $total_records === 0 ? 0 : (($current_page - 1) * ($per_page_val > 0 ? $per_page_val : 1)) + 1;
$to = $per_page_val > 0 ? min($current_page * $per_page_val, $total_records) : $total_records;
?>
